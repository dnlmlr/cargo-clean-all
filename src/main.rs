...
Examples:
    # Clean all projects in the current directory with a target-dir size of 
    # 500MB or more
    ~/projects $ cargo clean-all --keep-size 500MB

    # Clean all projects in the "~/projects" directory that were last compiled 
    # 7 or more days ago
...
fn parse_args() -> Result<AppArgs, pico_args::Error> {
    ...
    Ok(AppArgs {
        root_dir: pargs.opt_value_from_str("--dir")?
            .unwrap_or(".".to_string()),
        yes: pargs.contains("--yes"),
        keep_size: pargs.opt_value_from_fn("--keep-size", parse_arg_opt_filesize)?
            .unwrap_or(0),
        keep_last_modified: pargs.opt_value_from_str("--keep-days")?
            .unwrap_or(0)
    })
}
...
fn print_help_and_exit() {
    println!("cargo-clean-all v{}{}", env!("CARGO_PKG_VERSION"), HELP_TEXT);
    std::process::exit(0);
}
...
fn main() {
    ...
    let (mut projects, mut ignored): (Vec<_>, Vec<_>) = find_cargo_projects(scan_path)
        ...
        .partition(|it| {
            let secs_elapsed = it.last_modified
                .elapsed()
                .expect(&format!("Timestamp calculation failed: {}", it.project_path.display()))
                .as_secs_f32();
...
fn find_cargo_projects(path: &Path) -> Vec<PathBuf> {
    ...
    for it in path.read_dir().unwrap()
